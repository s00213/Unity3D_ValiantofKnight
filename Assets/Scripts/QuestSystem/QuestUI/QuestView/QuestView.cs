...
{
	[SerializeField] private QuestListViewController questListViewManager;
	...
	private void Start()
	{
		...
		foreach (var tab in questListViewManager.Tabs)
	...
	private void AddQuestToActiveListView(Quest quest)
		=> questListViewManager.AddQuestToActiveListView(quest, isOn => ShowDetail(isOn, quest));

	private void AddQuestToCompletedListView(Quest quest)
		=> questListViewManager.AddQuestToCompletedListView(quest, isOn => ShowDetail(isOn, quest));
	...
	{
		questListViewManager.RemoveQuestFromActiveListView(quest);
		if (questDetailView.Target == quest)
			questDetailView.Hide();
	}
}
